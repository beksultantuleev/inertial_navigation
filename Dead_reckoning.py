...
class Dead_Reckoning():
		...
		def simulate(self, localAcc, localAngular, delta_t):
			...
			new_global_acc = self.rotation_matrix.dot(self.localAcc) - self.gravity	
			# print('Accel')
...
if __name__=="__main__":
	...
	while step<100:
		...
		if len(magnetometer.pos)>0 and len(accelerometer.pos)>0 and len(gyroscope.pos)>0:
			time.sleep(0.1)
			# print(magnetometer.pos, step)
			
			
			test.simulate(accelerometer.pos, gyroscope.pos, delta_t)
			step+=1
